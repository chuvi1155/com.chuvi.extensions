...
/// - An address string for the device on the board youнre dealing with.
...
/// - subsystems с classes of device, such as analog inputs, servo controllers, and digital outputs.
/// - devices с the index of a specific device within a subsystem.  
/// If there is only one device in a subsystem, the device is not included in the OSC address.
/// - properties с different devices have different properties, such as the value of an analog input, 
...
/// corresponding structures are used с UsbPacket and UdpPacket.  Once youнve created 
/// a packet, you can simply call its Send() method, with the OscMessage youнd like to send.  
...
/// send an OscMessage with the address of the device youнd like to read, but omit 
...
public class UDPPacketIO
{
    ...
    ~UDPPacketIO()
    {
        ...
        if (IsOpen())
        {
            Console.WriteLine("closing udpclient listener on port " + localPort);
            Close();
        }
    ...
    public bool Open()
    {
        ...
        {
            Console.Error.WriteLine("cannot open udp client interface at port " + localPort);
    ...
    public void SendPacket(byte[] packet, int length)
    {
        ...
        if (!IsOpen())
        {
            Console.Error.WriteLine("[OSC ERROR] Cant send packet");
            return;
        }
...
public class OscMessage
{
    ...
    public int GetInt(int index)
    {
        ...
        {
            Console.Error.WriteLine("Wrong type");
            return 0;
        }
    }
...
public class OSC
{
    ...
    public int inPort = 6161;
    public string outIP = "127.0.0.1";
    public int outPort = 6969;
    ...
    bool isInit = false;
    public bool IsInit => isInit && OscPacketIO != null && OscPacketIO.IsOpen();
...
#endif
    static Dictionary<int, OSC> createdRecievers = new Dictionary<int, OSC>();
    static Dictionary<string, Dictionary<int, OSC>> createdSenders = new Dictionary<string, Dictionary<int, OSC>>();
    public static OSC CreateReciever(int port)
    {
        if (createdRecievers.ContainsKey(port))
            return createdRecievers[port];
        var osc = new OSC(port);
        createdRecievers.Add(port, osc);
        return osc;
    }
    public static OSC CreateSender(string ip, int port)
    {
        if (createdSenders.ContainsKey(ip) && createdSenders[ip].ContainsKey(port))
            return createdSenders[ip][port];
        var sender = new OSC(ip, port);
        if(!createdSenders.ContainsKey(ip))
        {
            var dict = new Dictionary<int, OSC> { {port, sender } };
            createdSenders.Add(ip, dict);
        }
        else
            createdSenders[ip].Add(port, sender);
        return sender;
    }
    ...
    public void Close()
    {
        ...
        if (ReaderRunning)
        {
            createdRecievers.Remove(inPort);
            ReaderRunning = false;
            ReadThread.Abort();
        }
        ...
        {
            if (createdSenders.ContainsKey(outIP))
            {
                createdSenders[outIP].Remove(outPort);
                if (createdSenders[outIP].Count == 0)
                    createdSenders.Remove(outIP);
            }
            OscPacketIO.Close();
            OscPacketIO = null;
    ...
    private void Read()
    {
                    ...
                    lock (ReadThreadLock)
                    {
                        if (paused == false)
                        {
                            ...
                            foreach (OscMessage msg in array)
                            {
                                ...
                                ArrayList al = (ArrayList)Hashtable.Synchronized(AddressTable)[msg.address];
                                if (al != null)
                                {
        ...
        {
            Console.WriteLine("ThreadAbortException" + e);
        }
        finally
        {
    ...
    private static int OscMessageToPacket(OscMessage oscM, byte[] packet, int start, int length)
    {
        ...
        //if (oscM.values.Count > 0)
        {
            ...
            foreach (object o in oscM.values)
            {
                ...
                {
                    tag.Append("b");
                    var bytes = System.BitConverter.GetBytes((bool)o);
    ...
    public void Send(string address, params object[] values)
    {
        if(values == null)
            values = new object[0];
        var msg = new OscMessage { address =  address, values = new ArrayList(values) };
        Send(msg);
    }
}
//}
